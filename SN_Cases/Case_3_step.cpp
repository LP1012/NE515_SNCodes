...
// Variable "flag" specifies whether the flux is on the right or left boundary; takes values of "R" and "L", respectively
{
    ...
    if (flag == "L")
    {
        ...
        return 4.0 * M_PI * term; // THIS DIFFERS FROM SN1 BY A FACTOR OF 2 TO NORMALIZE THE SOURCE
    }
    else if (flag == "R")
    {
        int N = mus.size();
...
int main(int argc, char const *argv[])
{
    ...
    double alpha = 0.0;                                  // specifies diamond differencing
    ...
    {
        gammas_L.push_back(Gamma_L(mus[k], mus[k])); // previously specified a single direction. Now specifies all positive directions
    ...
    for (int I : n_cells)
    {
        ...
        output_filename = to_string(I) + "_case3_step_out.csv";
